...
TEST_F(DepsLogTest, WriteRead) {
...

  {
    ...
    log1.RecordDeps(state1.GetNode("out.o", 0), TimeStamp(TimeStamp::duration(1)), deps);

    deps.clear();
    deps.push_back(state1.GetNode("foo.h", 0));
    deps.push_back(state1.GetNode("bar2.h", 0));
    log1.RecordDeps(state1.GetNode("out2.o", 0), TimeStamp(TimeStamp::duration(2)), deps);
...
TEST_F(DepsLogTest, DoubleEntry) {
  ...
  int file_size;
  {
    ...
    log.RecordDeps(state.GetNode("out.o", 0), TimeStamp(TimeStamp::duration(1)), deps);
    log.Close();
...
"build other_out.o: cc\n";
  ...
  int file_size;
  {
    ...
    log.RecordDeps(state.GetNode("other_out.o", 0), TimeStamp(TimeStamp::duration(1)), deps);
...
TEST_F(DepsLogTest, Truncated) {
  // Create a file with some entries.
  {
    ...
    log.RecordDeps(state.GetNode("out2.o", 0), TimeStamp(TimeStamp::duration(2)), deps);
...
TEST_F(DepsLogTest, TruncatedRecovery) {
  ...
  // Load the file again, add an entry.
  {
    ...
    deps.push_back(state.GetNode("foo.h", 0));
    deps.push_back(state.GetNode("bar2.h", 0));
    log.RecordDeps(state.GetNode("out2.o", 0), TimeStamp(TimeStamp::duration(3)), deps);
